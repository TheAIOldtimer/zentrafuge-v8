...
import { showStatusMessage, setFormEnabled, appendMessage } from './ui.js';
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
      ...
      if (user) {
        try {
          ...
          if (window.location.pathname.includes('index.html')) {
            ...
            showStatusMessage(t.welcomeBack, 'success');
            setTimeout(() => {
          ...
          showStatusMessage(t.loginFailed, 'error');
          await firebase.auth().signOut();
    ...
    if (form) {
      console.log('🎯 Found chat-form, setting up event listeners');
      ...
      form.addEventListener("submit", async (e) => {
        ...
        if (!isAuthorized) {
          showStatusMessage('You are not authorized to chat. Please sign in and complete onboarding.', 'error');
          return;
        }
        ...
        while (attempt <= maxAttempts && !success) {
          try {
            ...
            console.log('🔍 Index fetch response status:', res.status);
      ...
      console.warn('⚠️ No chat-form found for event listeners');
    }
    
    if (document.getElementById('language-style')) {
      console.log('🔍 Initializing preferences form');
      await loadUserPreferences();
    ...
    showStatusMessage('Failed to initialize app. Please try again.', 'error');
